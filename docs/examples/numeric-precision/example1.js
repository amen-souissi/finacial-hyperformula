...
// RoundingMode.ROUND_HALF_EVEN = 6 (Banker's rounding)
 ...
 */
function renderInvoiceTable(hf, containerId, usePrecision) {
...
function renderPrecisionTests() {
  ...
  for (let row = 1; row < height; row++) {
    ...
    const preciseResult = hfPrecise.getCellValue({ sheet: sheetId, col: 1, row }, { keepNumeric: true });
    ...
    newTbodyHTML += `<tr>
      ...
      <td class="precise">${preciseResult}</td>
    </tr>`;
  }
