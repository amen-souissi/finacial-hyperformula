...
hfPrecise.setSheetContent(
  hfPrecise.getSheetId('PrecisionTests'),
  precisionTestData
);
...
hfNative.setSheetContent(
  hfNative.getSheetId('PrecisionTests'),
  precisionTestData
);
    ...
    for (let col = 0; col < width; col++) {
      ...
      if (!hf.isCellEmpty(cellAddress)) {
        if (usePrecision && hf.doesCellHaveFormula(cellAddress)) {
          ...
          cellValue = hf.getCellValue(cellAddress, { keepNumeric: true });
        } else {
          // Use native number (default behavior)
...
function renderPrecisionTests() {
  ...
  for (let row = 1; row < height; row++) {
    ...
    // With keepNumeric: string for full precision
    const preciseResult = hfPrecise.getCellValue(
      { sheet: sheetId, col: 1, row },
      { keepNumeric: true }
    );
    // Default: native JavaScript number
    const nativeResult = hfNative.getCellValue({
      sheet: hfNative.getSheetId('PrecisionTests'),
      col: 1,
      row,
    });

    const isCorrect =
      preciseResult === description.toString().split(' ')[0] ||
