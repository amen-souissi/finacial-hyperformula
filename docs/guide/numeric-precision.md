...
// Get Numeric object with full precision
...
// Or get native number (default)
console.log(hf.getCellValue({ sheet: 0, row: 0, col: 2 })); // 0.3
```
...
```javascript
const hf = HyperFormula.buildFromArray([['0.1', '0.2', '=A1+B1']], {
  licenseKey: 'gpl-v3',
  numericImplementation: 'native',
});

// Get as native JavaScript number (default behavior)
...
// Get results as Numeric objects with full precision
const subtotal = hf.getCellValue(
  { sheet: 0, row: 4, col: 3 },
  { keepNumeric: true }
);
const tax = hf.getCellValue(
  { sheet: 0, row: 5, col: 3 },
  { keepNumeric: true }
);
const total = hf.getCellValue(
  { sheet: 0, row: 6, col: 3 },
  { keepNumeric: true }
);
...
// Or use Numeric methods for further calculations
const roundedTax = tax.toDecimalPlaces(2); // Round to 2 decimal places
```
...
### 2. Use keepNumeric option for output

By default, `getCellValue` returns native JavaScript numbers. Use `{ keepNumeric: true }` to get `Numeric` objects that preserve full precision:

```javascript
// Get as native JavaScript number (default, may lose precision)
const numericValue = hf.getCellValue({ sheet: 0, row: 0, col: 0 });

// Get Numeric object with full precision
const preciseValue = hf.getCellValue(
  { sheet: 0, row: 0, col: 0 },
  { keepNumeric: true }
);

// Numeric objects can be used for further calculations or converted
console.log(preciseValue.toString()); // "0.3" - exact string
console.log(preciseValue.toNumber()); // 0.3 - native number
console.log(preciseValue.plus(anotherNumeric)); // Precise addition
```
