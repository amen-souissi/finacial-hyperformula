...
// Helper to convert complex [Numeric, Numeric] to [number, number] for comparison
const toComplexNum = (val: unknown): unknown => {
  if (val instanceof CellError) {
    return val
  }
  if (Array.isArray(val) && val.length === 2) {
    return [val[0].toNumber(), val[1].toNumber()]
  }
  return val
}
...
describe('#coerceScalarToComplex', () => {
  ...
  const arithmeticHelper = new ArithmeticHelper(config, dateTimeHelper, numberLiteralsHelper)
  it('works', () => {
    expect(toComplexNum(arithmeticHelper.coerceScalarToComplex(42))).toEqual([42, 0])
    expect(toComplexNum(arithmeticHelper.coerceScalarToComplex(true))).toEqual(new CellError(ErrorType.NUM, ErrorMessage.ComplexNumberExpected))
    expect(toComplexNum(arithmeticHelper.coerceScalarToComplex(EmptyValue))).toEqual([0, 0])
    expect(toComplexNum(arithmeticHelper.coerceScalarToComplex(''))).toEqual(new CellError(ErrorType.NUM, ErrorMessage.ComplexNumberExpected))
    expect(toComplexNum(arithmeticHelper.coerceScalarToComplex('1'))).toEqual([1, 0])
    expect(toComplexNum(arithmeticHelper.coerceScalarToComplex('-1.1'))).toEqual([-1.1, 0])
    expect(toComplexNum(arithmeticHelper.coerceScalarToComplex('+.1'))).toEqual([0.1, 0])
    expect(toComplexNum(arithmeticHelper.coerceScalarToComplex('1e1'))).toEqual([10, 0])
    expect(toComplexNum(arithmeticHelper.coerceScalarToComplex('1 i'))).toEqual([0, 1])
    expect(toComplexNum(arithmeticHelper.coerceScalarToComplex('-1.1j'))).toEqual([0, -1.1])
    expect(toComplexNum(arithmeticHelper.coerceScalarToComplex('+.1 i'))).toEqual([0, 0.1])
    expect(toComplexNum(arithmeticHelper.coerceScalarToComplex('1e1j'))).toEqual([0, 10])
    expect(toComplexNum(arithmeticHelper.coerceScalarToComplex('i'))).toEqual([0, 1])
    expect(toComplexNum(arithmeticHelper.coerceScalarToComplex('-i'))).toEqual([0, -1])
    expect(toComplexNum(arithmeticHelper.coerceScalarToComplex('+i'))).toEqual([0, 1])
    expect(toComplexNum(arithmeticHelper.coerceScalarToComplex('i1'))).toEqual(new CellError(ErrorType.NUM, ErrorMessage.ComplexNumberExpected))
    expect(toComplexNum(arithmeticHelper.coerceScalarToComplex('--1'))).toEqual(new CellError(ErrorType.NUM, ErrorMessage.ComplexNumberExpected))
    expect(toComplexNum(arithmeticHelper.coerceScalarToComplex('i+1'))).toEqual(new CellError(ErrorType.NUM, ErrorMessage.ComplexNumberExpected))
    expect(toComplexNum(arithmeticHelper.coerceScalarToComplex('1+-i'))).toEqual([1, -1])
    expect(toComplexNum(arithmeticHelper.coerceScalarToComplex('0.1+.1 i'))).toEqual([0.1, 0.1])
    expect(toComplexNum(arithmeticHelper.coerceScalarToComplex(' - 1.0e+1 - - 1.0e+1j'))).toEqual([-10, 10])
    expect(toComplexNum(arithmeticHelper.coerceScalarToComplex(new CellError(ErrorType.DIV_BY_ZERO)))).toEqual(new CellError(ErrorType.DIV_BY_ZERO))
  })
})
