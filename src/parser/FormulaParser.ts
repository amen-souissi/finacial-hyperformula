...
export class FormulaParser extends EmbeddedActionsParser {
  ...
  private handleOffsetHeuristic(args: Ast[]): Ast {
    ...
    // OFFSET row/column shifts - these are verified integers used as array indices.
    // No precision impact: integers up to 2^53-1 are exact in JavaScript.
    const rowsArg = args[1]
    let rowShift: number
    if (rowsArg.type === AstNodeType.NUMBER && rowsArg.value.isInteger()) {
      rowShift = rowsArg.value.toNumber()  // Safe: verified integer for array index
    } else if (rowsArg.type === AstNodeType.PLUS_UNARY_OP && rowsArg.value.type === AstNodeType.NUMBER && rowsArg.value.value.isInteger()) {
      rowShift = rowsArg.value.value.toNumber()  // Safe: verified integer for array index
    ...
    if (columnsArg.type === AstNodeType.NUMBER && columnsArg.value.isInteger()) {
      colShift = columnsArg.value.toNumber()  // Safe: verified integer for array index
      ...
      colShift = columnsArg.value.value.toNumber()  // Safe: verified integer for array index
    ...
    // OFFSET height/width - these are verified integers used as dimensions.
    // No precision impact: integers up to 2^53-1 are exact in JavaScript.
    ...
    if (heightArg === undefined) {
      ...
      const heightVal = heightArg.value.toNumber()  // Safe: will be verified as integer below
      if (heightVal < 1) {
    ...
    if (widthArg === undefined) {
      ...
      const widthVal = widthArg.value.toNumber()  // Safe: will be verified as integer below
      if (widthVal < 1) {
