...
export class ColumnIndex implements ColumnSearchStrategy {
  ...
  private normalizeValueForIndex(value: RawInterpreterValue): RawInterpreterValue {
    ...
    // Note: This conversion is necessary because JavaScript Maps use reference equality for objects.
    // The precision loss is acceptable here as lookup keys typically don't need high precision.
    if (isExtendedNumber(value)) {
      return getRawPrecisionValue(value).toNumber()
    }
    return value
  }
