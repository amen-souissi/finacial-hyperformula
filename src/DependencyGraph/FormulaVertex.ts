...
export class ArrayFormulaVertex extends FormulaVertex {
  ...
  /**
   * Get raw cell content for display/export purposes.
   * 
   * OUTPUT LAYER: This converts internal Numeric to native number for raw content access.
   * Raw content is used for display and serialization, not for calculations.
   */
  getArrayCellRawValue(address: SimpleCellAddress): Maybe<RawCellContent> {
    ...
    if (val instanceof CellError || val === EmptyValue) {
      ...
      // OUTPUT: Convert to native number for raw content (display/export)
