...
import {EmptyValue, getRawPrecisionValue, getRawValue, isExtendedNumber, RawInterpreterValue, RawNoErrorScalarValue} from './InterpreterValue'
...
export function compare(left: RawNoErrorScalarValue, right: RawInterpreterValue): number {
  ...
  // Both are numbers (native or Numeric) - use Numeric comparison
  if (leftIsNumber && rightIsNumber) {
    const leftNumeric = getRawPrecisionValue(left as any)
    const rightNumeric = getRawPrecisionValue(right as any)
    return leftNumeric.comparedTo(rightNumeric)
  }
